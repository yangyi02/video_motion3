...
class Net(nn.Module):
    ...
    def forward(self, im_input_f, im_input_b):
        ...
        d_mask_f = F.softmax(self.conv_d(x11))

        im = im_input_b[:, -self.im_channel:, :, :]
        x = self.bn0_d(self.conv0_d(im))
        x1 = F.relu(self.bn1_d(self.conv1_d(x)))
        x2 = self.maxpool(x1)
        x2 = F.relu(self.bn2_d(self.conv2_d(x2)))
        ...
        x11 = torch.cat((x10, x1), 1)
        x11 = F.relu(self.bn11_d(self.conv11_d(x11)))
        d_mask_b = F.softmax(self.conv_d(x11))

        out_mask_f = construct_mask(m_mask_f, d_mask_f, self.m_kernel, self.m_range)
        im = im_input_f[:, -self.im_channel:, :, :]
        pred_f = construct_image(im, out_mask_f, self.m_kernel, self.m_range)

        out_mask_b = construct_mask(m_mask_b, d_mask_b, self.m_kernel, self.m_range)
        im = im_input_b[:, -self.im_channel:, :, :]
        pred_b = construct_image(im, out_mask_b, self.m_kernel, self.m_range)

        seg_f = out_mask_f.sum(1).unsqueeze(1)
        seg_b = out_mask_b.sum(1).unsqueeze(1)
        attn_f = (seg_f + 1e-8) / (seg_f + seg_b + 2e-8)
        attn_b = 1 - attn_f
        pred = pred_f * attn_f + pred_b * attn_b
        return pred, m_mask_f, d_mask_f, attn_f, m_mask_b, d_mask_b, attn_b
...
class GtNet(nn.Module):
    ...
    def forward(self, im_input_f, im_input_b, gt_motion_f, gt_depth_f, gt_motion_b, gt_depth_b, gt_type=None):
        if gt_type == 'label':
            m_mask_f = self.label2mask(gt_motion_f)
            m_mask_b = self.label2mask(gt_motion_b)
        else:
            m_mask_f = self.motion2mask(gt_motion_f)
            m_mask_b = self.motion2mask(gt_motion_b)
        d_mask_f = self.depth2mask(gt_depth_f)
        d_mask_b = self.depth2mask(gt_depth_b)
