...
class Demo(BaseDemo):
    ...
    def train(self):
        ...
        for epoch in range(self.train_epoch):
            ...
            im_pred, m_mask, d_mask, appear = self.model(im_input, im_output)
    ...
    def test_gt(self):
        ...
        for epoch in range(self.test_epoch):
            ...
            if self.data.name in ['box', 'mnist', 'box_complex']:
                im_pred, m_mask, d_mask, appear = self.model_gt(im_input, im_output, gt_motion_label, gt_depth, 'label')
            elif self.data.name in['box2', 'mnist2']:
                im_pred, m_mask, d_mask, appear = self.model_gt(im_input, im_output, gt_motion, gt_depth)
            im_diff = im_pred - im_output
            loss = torch.abs(im_diff).sum()
