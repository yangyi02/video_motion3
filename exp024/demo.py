...
class Demo(BaseDemo):
    ...
    def train(self):
        ...
        for epoch in range(self.train_epoch):
            ...
            im_pred, _ = self.model(im_input)
            im_diff = im_pred - im_output  # inverse warping loss
            loss = torch.max(torch.abs(im_diff), 1)[0].sum()
            loss.backward()
            optimizer.step()
            ...
            base_loss.append(torch.max(torch.abs(im_input[:, -self.im_channel:, :, :] - im_output), 1)[0].sum().data[0])
    ...
    def test(self):
        ...
        for epoch in range(self.test_epoch):
            ...
            if torch.cuda.is_available():
                im_input, im_output = im_input.cuda(), im_output.cuda()
            im_pred, flow = self.model(im_input)
            im_diff = im_pred - im_output  # inverse warping loss
            loss = torch.max(torch.abs(im_diff), 1)[0].sum()
    ...
    def test_gt(self):
        ...
        for epoch in range(self.test_epoch):
            ...
            if torch.cuda.is_available():
                ...
                gt_motion = gt_motion.cuda()
            if self.data.name in ['box', 'mnist', 'box_complex']:
                im_pred, flow = self.model_gt(im_input, gt_motion)
            elif self.data.name in['box2', 'mnist2']:
                im_pred, flow = self.model_gt(im_input, gt_motion)
            im_diff = im_pred - im_output  # inverse warping loss
            loss = torch.max(torch.abs(im_diff), 1)[0].sum()
